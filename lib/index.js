...
class App
{
    ...
    setConfig()
    {
        ...
        // Custom
        config.domain = `https://${config.host ? config.host : ip.address()}${config.port !== 80 ? ':' + config.port : ''}`
    ...
    setSocket()
    {
        this.config.socket = socketIo.listen(this.site.server, { path: '/keppler/socket.io' })
